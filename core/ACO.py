...
class Ant:
    ...
    def move(self, dire):
        ...
        if [self.x, self.y] not in known_list:
            known_list.append([self.x, self.y])
        if self.x-1 > 0 and self.y-1 > 0:
            if [self.x-1, self.y-1] not in known_list:
                known_list.append([self.x-1, self.y-1])
        if self.y-1 > 0:
            if [self.x, self.y-1] not in known_list:
                known_list.append([self.x, self.y-1])
        if self.x+1 < maze.num_x and self.y-1 > 0:
            if [self.x+1, self.y-1] not in known_list:
                known_list.append([self.x+1, self.y-1])
        if self.x-1 > 0:
            if [self.x-1, self.y] not in known_list:
                known_list.append([self.x-1, self.y])
        if self.x+1 < maze.num_x:
            if [self.x+1, self.y] not in known_list:
                known_list.append([self.x+1, self.y])
        if self.x-1 > 0 and self.y+1 < maze.num_y:
            if [self.x-1, self.y+1] not in known_list:
                known_list.append([self.x-1, self.y+1])
        if self.y+1 < maze.num_y:
            if [self.x, self.y+1] not in known_list:
                known_list.append([self.x, self.y+1])
        if self.x+1 < maze.num_x and self.y+1 < maze.num_y:
            if [self.x+1, self.y+1] not in known_list:
                known_list.append([self.x+1, self.y+1])
        dx, dy = 0, 0
    ...
    def turn(self):
        ...
        if not self.find_food:
            ...
            for dire in self.possible_dires:
                total += (self.getPheromone(dire) ** alpha) * \
                    (self.getInverseDistance(dire) ** beta)
            for dire in self.possible_dires:    # 整个分数
                probabilities.append((self.getPheromone(
                    dire) ** alpha) * (self.getInverseDistance(dire) ** beta) / total)
    ...
    def getInverseDistance(self, dire):
        ...
        # 知道终点
        return float(1 / (0.1+((maze.foodX - x) ** 2 + (maze.foodY - y) ** 2) ** 0.5))
        ''' # 不知道终点
...
def createAnts():
    ...
    # elite_ratio = float(input("elite_ratio:"))
