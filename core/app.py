...
class App:
    def __init__(self, WIDTH, HEIGHT, TEXT_HEIGHT, SCALE):
        ...
        pygame.draw.line(self.screen, self.BLACK,
                         (self.WIDTH/2, 0), (self.WIDTH/2, self.HEIGHT), 3)
        ...
        for i in range(maze.num_x):  # 画信息素，出生点，食物点，墙
            for j in range(maze.num_y):
                ...
                if type(matrix[i][j]) != str:   # 信息素
                    # RGB,R、B分量.信息素越多颜色越绿
                    grey = 255 - (matrix[i][j] - maze.pheromone_init) * 50.0
                    green = 2*grey
                    if grey > 255:
                        grey = 255
                    if grey < 0:
    ...
    def part_Map(self):
        for i, j in ACO.known_list:  # 画信息素，出生点，食物点，墙
            if type(matrix[i][j]) != str:   # 信息素
                # RGB,R、B分量.信息素越多颜色越绿
                grey = 255 - (matrix[i][j] - maze.pheromone_init) * 50.0
                green = 2*grey
        ...
        for i in range(maze.num_x):  # 画信息素，出生点，食物点，墙
            for j in range(maze.num_y):
                ...
                if matrix[i][j] == "wall":    # 墙
                    self.drawPoint(self.YELLOW, i, j, False, True)
    # 画地图
    ...
    def drawMap(self):
        ...
        self.drawPoint(self.LIGHTBLUE, maze.spawnX,
                       maze.spawnY, True, True)   # 出生点
    ...
    def drawPoint(self, color, x, y, left, right):
        ...
        if right:   # 画右边
            self.screen.blit(
                surf, (self.WIDTH/2+x*self.SCALE, y*self.SCALE))  # 显示位置
    ...
    def showText(self, text):
        ...
        font = pygame.font.SysFont("arial", int(self.TEXT_HEIGHT/3))  # 字体和大小
